...
- Metadata: **SQLite** (driver **`node:sqlite`** built-in của Node — `DatabaseSync`, chạy ở main process) — đơn file, backup dễ.
  > **Cập nhật 2026-07-11 (feature 009-notebooks):** đổi driver từ `better-sqlite3` → **`node:sqlite`**.
  > Lý do: `node:sqlite` (API sync giống better-sqlite3) đã có sẵn trong CẢ Electron 43 main lẫn Node 24
  > (vitest) — **không cần native module, không rebuild ABI theo Electron**, bỏ hẳn friction cài đặt.
  > Đã probe xác nhận chạy ở cả 2 runtime. Đổi stack này ghi tại đây theo quy tắc "ADR-governed stack".
